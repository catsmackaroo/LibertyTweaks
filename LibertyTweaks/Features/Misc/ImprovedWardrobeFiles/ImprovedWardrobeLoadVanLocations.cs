...
public class ImprovedWardrobeLoadVanLocations
{
    public List<VanLocation> vanLocations;
    public List<VanLocation> GetVanLocations()
    {
        return vanLocations;
    }

    public ImprovedWardrobeLoadVanLocations()
    {
        vanLocations = new List<VanLocation>();
        LoadTeleportLocationsFromFile("IVSDKDotNet/scripts/LibertyTweaks/ImprovedWardrobeFiles/ClothesVanLocations.txt");
    }
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (parts.Length >= 4)
                    {
                        ...
                        {
                            vanLocations.Add(new VanLocation(x, y, z, heading));
                        }
                        else
                        {
    ...
    public VanLocation GetNearestTeleportLocation(Vector3 playerPosition)
    {
        VanLocation nearestLocation = null;
        float minDistance = float.MaxValue;

        foreach (var location in vanLocations)
        {
...
public class VanLocation
{
    ...
    public VanLocation(float x, float y, float z, float heading)
    {
        X = x;
        Y = y;
        Z = z;
